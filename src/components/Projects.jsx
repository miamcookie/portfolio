      ...
      <div className="modal" onClick={e => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        {project.images && project.images.length > 0 ? (
          ...
          <div className="modal__no-img">
            <span>{t.projects.noImg}</span>
          </div>
        )}
        <p className="modal__desc">{project.details}</p>
        <div className="modal__tech-wrap">
          <span className="modal__tech-label">{t.projects.stackLabel}</span>
          <ul className="modal__tech">
            {project.tech.map((tech) => (
              <li key={tech} className="modal__badge" style={{ '--badge-color': project.color }}>
                {tech}
              </li>
            ))}
          </ul>
